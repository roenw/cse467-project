...
# Run original TF1-style PATEGAN code on TensorFlow 2.x.
...
def pate_lamda (x, teacher_models, lamda):
  ...
  # Keep vote counts as scalar ints to avoid array/scalar shape issues downstream.
...
def pategan(x_train, parameters):
  ...
  ## Iterations
  pbar = tqdm(desc='Generating synthetic data', unit='iteration')
  while epsilon_hat < epsilon:      
    ...
    pbar.update(1)
    pbar.set_postfix({'epsilon': f'{epsilon_hat:.4f}'})    
